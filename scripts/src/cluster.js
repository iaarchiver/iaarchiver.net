...
var cluster = {
	 ...
	 shellac : function(object, styles){ 
		  ...
		  function getStyle(styleProp) {
			  ...
			  var text = object.innerHTML.replace(/(<[^>]*>).*?(<\/[^>]*>)/gm,'');
			  ...
			  for(var i=0;i<phraseArray.length;i++) {
				  context.fillText(phraseArray[0], 0, (i * styles.leading) + styles.size);
			  }
			return canvas;
	}, 
	...
	Node : function(x,y,parent) {
		...
		t.el.setAttribute('cy', y+parent.offset)
		...
		var changeOP = function(){
			...
			clearTimeout(drawTO);
			drawTO = setTimeout(parent.callb,400)
		}
...
$.fn.cluster = function(){
	if (!this[0]){
		//console.log('nothing clustered');
		return;
	}
	var target = this,
		id = this.attr('id');
	cluster.callb = function(){
